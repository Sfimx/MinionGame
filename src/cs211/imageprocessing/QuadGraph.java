...
public class QuadGraph {
    ...
    public List<int[]> findCycles() {
        ...
        for (int i = 0; i < graph.length; i++) 
        {
            ...
            {
                findNewCycles(new int[] {graph[i][j]});//either first or second line of intersection
            }
        }
        for (int[] cy : cycles) 
        {
    ...
    public void findNewCycles(int[] path)
    {
            ...
            //scan all the "lines" (nodes) of the intersections(edges) in graph (list of pair of nodes => edges)
            for (int i = 0; i < graph.length; i++)//for all intersection(edges) in graph
                for (int y = 0; y < 2; y++)//
                    if (graph[i][y] == n)//if scanned line(node) == n
                    //  => edge refers to our current node
                    {
                        x = graph[i][(y + 1) % 2];//x == other line(node) in intersection(edge)
                        if (!visited(x, path) )
                        //  neighbor node not on path yet
                        {
                            sub[0] = x;
                            System.arraycopy(path, 0, sub, 1, path.length);//sub <= x+:path
                            //  explore extended path
                            findNewCycles(sub);
                        }
                        else if ((path.length == 4) && (x == path[path.length - 1]))
                        //  cycle found (only cycles of lenght 4)
                        {
